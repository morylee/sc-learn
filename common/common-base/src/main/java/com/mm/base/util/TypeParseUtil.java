...
import com.mm.base.exception.DataParseException;
import com.mm.base.support.DataType;
...
//			return getNf(locale).format(intgr.toString());
	...
	private static Object double2Obj(Object value, String type, Locale locale) {
		...
		if ("String".equalsIgnoreCase(type) || DataType.STRING.equalsIgnoreCase(type)) {
			...
			return Integer.parseInt(new java.text.DecimalFormat("0").format(new BigDecimal(dbl).setScale(0, BigDecimal.ROUND_HALF_UP)));
		} else if ("BigDecimal".equalsIgnoreCase(type) || DataType.BIGDECIMAL.equalsIgnoreCase(type)) {
	...
	/** 转换为整型 */
	public static final Integer convertToInteger(Object obj, Integer defaultValue) {
		Object i = convert(obj, "String", null);
		if (i != null) {
			return (Integer) i;
		} else {
			return defaultValue;
